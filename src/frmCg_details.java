...
class frmCg_details extends JFrame implements ActionListener
  {
  JLabel  lblTitle;
  JLabel  lblBB_NO;
  JLabel  lblGUEST_ID;
  JLabel  lblGUEST_NAME;
  JLabel  lblDIR_EX_NAME;
  JLabel  lblARR_FROM;
  JLabel  lblPUR_VISIT;
  JLabel  lblARR_DATE;
  JLabel  lblTIME;
  ...
  try{
      ...
      rs=st.executeQuery("SELECT BB_NO_NO,GUEST_ID,GUEST_NAME,DIR_EX_NAME,ARR_FROM,PUR_VISIT,ARR_DATE,TIME,DEP_DATE,NO_PERSON,WHERE_TO_GO,COM_NAME,S_NO_NAME,LOCALITY,PIN,CITY, STATE,OFF_PHONE,H_PHONE,FAX_NO,EMAIL_ID,ROOM_NO,ROOM_CHARGE,ADV,DUES,RECEP_CODE,PAY_MODE FROM COMPANY_GUEST_DETAILS ");
  ...
  public frmCg_details()
  {
    setTitle("IMSHSE,Shirur-pune,Maharashtra");
    setSize(780,580);
    cnt=getContentPane();
    cnt.setLayout(null);

    lblTitle    = new JLabel("Company Guest Details");
    ...
    lblDIR_EX_NAME     = new JLabel("DIR EX NAME");
    ...
    lblCOM_NAME    = new JLabel("COM NAME");
    ...
    lblDIR_EX_NAME.setBounds(100,135,100,25);
    lblDIR_EX_NAME.setFont(new Font("Arial",Font.BOLD+Font.ITALIC,14));
    lblDIR_EX_NAME.setForeground(Color.black);
    ...
    lblCOM_NAME.setBounds(100,335,100,25);
    lblCOM_NAME.setFont(new Font("Arial",Font.BOLD+Font.ITALIC,14));
    lblCOM_NAME.setForeground(Color.black);
    ...
    txtDIR_EX_NAME   = new JTextField(25);
    ...
    txtDIR_EX_NAME.setBounds(230,135,155,20);
    ...
    txtCOM_NAME.setBounds(230,335,150,20);
    ...
    if(e.getSource()==btnFIND)
    {
      ...
      try{
      Statement st=cn.createStatement();
      ResultSet rs1=st.executeQuery("SELECT * FROM COMPANY_GUEST_DETAILS WHERE GUEST_ID="+fGUEST_ID);
      if(rs1.next())
      {
 ...
 st.close();
  ...
  public void showRecord()
  {
    try{         
      ...
      txtDIR_EX_NAME.setText(rs.getString(4));
      ...
      txtCOM_NAME.setText(rs.getString(12));
  ...
  public void enableEntry()
  {
      ...
      txtDIR_EX_NAME.setEditable(true);
  ...
  public void disableEntry()
  {
      ...
      txtDIR_EX_NAME.setEditable(false);
  ...
  public boolean saveRecord()
  {
    boolean result=true;
    try{
    String sql  = "INSERT INTO COMPANY_GUEST_DETAILS VALUES("+
                  txtBB_NO.getText() + "," +
                  "'" + txtGUEST_ID.getText() + "'," +
                  "'" + txtGUEST_NAME.getText() + "'," +
                  "'" + txtDIR_EX_NAME.getText() + "'," +
                  "'"+ txtARR_FROM.getText()+ "',"+
 ...
 public boolean updateRecord()
  {
    boolean result=true;
    try{
    String sql  = "UPDATE COMPANY_GUEST_DETAILS SET "+
                  txtBB_NO.getText() + "," +
                  "'" + txtGUEST_ID.getText() + "'," +
                  "'" + txtGUEST_NAME.getText() + "'," +
                  "'" + txtDIR_EX_NAME.getText() + "'," +
